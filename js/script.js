const ApiKey = "e87659e6ec0b4ca9817cddfc8f2ee767";
...
function getListTeams() {
    ...
    fetch(teamEndPoin, fetchHeader)
        ...
        .then(resJson => {
            ...
            detil.forEach(btn => {
                btn.onclick = (event) => {
                    getCardForDetail(baseUrl + "teams/" + event.target.dataset.id);
                    // console.log(baseUrl + "teams/" + event.target.dataset.id);
                }
            })
        }).catch(err => {
            console.error(err);
...
function getCardForDetail(id){
    fetch(id, fetchHeader)
        .then(response => response.json())
        .then(resJson => {
            // console.log(resJson);
            modalView.innerHTML = `
                <div class="card-image" style="display: flex; align-items: center; justify-content: center; margin: 10px 0px 0px 0px;">
                    <img class="activator" src="${resJson.crestUrl}">
                </div>
                <div class="modal-content">
                    <h4>${resJson.name}</h4>
                    <p>
                        Area : ${resJson.area.name} <br>
